//Sort employee records by salary 

#include <stdio.h>

// global or large temporary array to store merge elements 
int temp[100000];

// function to merge two halves of the array 
void merge(int arr[], int st, int mid, int end) {
    int i = st;       // starting index for the left half
    int j = mid + 1;  // starting index for the right half
    int k = st;       // starting index for the temporary array

    // compare and copy elements from both halves in sorted order
    while (i <= mid && j <= end) {
        if (arr[i] <= arr[j]) {
            temp[k] = arr[i];
            i++;
        } 
        else {
            temp[k] = arr[j];
            j++;
        }
        k++;
    }

    // copy any remaining elements of the left half
    while (i <= mid) {
        temp[k] = arr[i];
        i++;
        k++;
    }

    // copy any remaining elements of the right half 
    while (j <= end) {
        temp[k] = arr[j];
        j++;
        k++;
    }

    // copy the sorted elements back into the original array 
    for (i = st; i <= end; i++) {
        arr[i] = temp[i];
    }
}

// function to recursively split the array 
void mergesort(int arr[], int st, int end) {
    if (st < end) {
        int mid = (st + end) / 2; // find the middle value

        mergesort(arr, st, mid);      // sort the left half
        mergesort(arr, mid + 1, end); // sort the right half

        merge(arr, st, mid, end);     // merge the two halves
    }
}

int main() {
    int n;6

    // read the number of employees 
    printf("enter the number of employees:");
    scanf("%d",&n);

    int salaries[n]; // create an array named salaries

    // read the employee salaries 
    for (int i = 0; i < n; i++) {
        printf("enter the salary of %d employee:",i+1);
        scanf("%d", &salaries[i]);
    }

    // call merge sort on the entire array 
    mergesort(salaries, 0, n - 1);

    // print the sorted salaries separated by spaces 
    for (int i = 0; i < n; i++) {
        printf("%d", salaries[i]);
        if (i < n - 1) {
            printf(" ");
        }
    }
    printf("\n");

    // extract required stats directly from the sorted array
    int lowest = salaries[0];
    int highest = salaries[n - 1];
    int difference = highest - lowest;

    // print the final required metrics
    printf("Lowest Salary: %d\n", lowest);
    printf("Highest Salary: %d\n", highest);
    printf("Difference: %d\n", difference);

    return 0;
}